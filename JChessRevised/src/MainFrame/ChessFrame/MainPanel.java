        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if (!GameOver) {
                ...
                if (P1.GetInhand() != -1) {
                    ...
                    newP.x++;
                    newP.y++;
                    P1.getPieceOldPosition(P1.GetInhand());
                    ...
                    if (Iam_Server || local) {
                        ...
                        if (P1.GetInhand() < 33 && P1.GetInhand() > 24) {
                            for (int i = 1; i < 17; i++) {
                                samePostion = P2.getPositionOfPiece(i);
                        ...
                        if (!(newP.x == present.x && newP.y == present.y)/* &&!P1.returncheckKing() */) {
                            if (P1.checkthemove(newP, P1.GetInhand())) // if the move is illegal
                            {
                                ...
                                for (int i = 1; i <= 32; i++) {
                                    ...
                                    {
                                        if (i < 17) {
                                            flag = P1.checktheWay(newP, P2.getPositionOfPiece(i), P1.GetInhand());// Means
                                            // there is
                                            // somting in
                                            // the Way so
                                            // can't move
                ...
                else if (P2.GetInhand() != -1)// white
                {
                    ...
                    if (Iam_Client || local) {
                        ...
                        Point present = P2.getPositionOfPiece(P2.GetInhand());
                        ...
                        if (!(newP.x == present.x && newP.y == present.y)/* &&!P2.returncheckKing() */) {
                            if (P2.checkthemove(newP, P2.GetInhand())) {
                                ...
                                for (int i = 1; i <= 32; i++) {
                                    if (P2.GetInhand() != i) {
                                        if (i < 17) {
                                            flag = P2.checktheWay(newP, P2.getPositionOfPiece(i), P2.GetInhand());
                                        } else {
                                ...
                                if (!flag) {
                                    P2.getPositionOfPiece(8);
                                    Point myold = new Point();
                                    Point o = P2.getPositionOfPiece(P2.GetInhand());
                                    myold.x = o.x;
                                    myold.y = o.y;
                                    Point other = new Point();
                                    Point f = new Point();
    ...
    public boolean BoardgetPostion(int x, int y) {
        if (!GameOver && Game_started) {
            if ((Iam_Server && players_turn == 1) || (local) || (Iam_Client && players_turn == 2)) {
                ...
                {
                    for (int i = 1; i <= 16; i++) {
                        Point p = P2.getPositionOfPiece(i);
                        if (p.x == newX && p.y == newY) {
                            P2.SetInhand(i);
                            whenHandleAndPice(x, y);
                            return true;
        ...
        for (int i = 1; i <= 32; i++) {
            if (i < 17) {
                if (i == P2.GetInhand()) {
                    ...
                    postionPoint = P2.getPositionOfPiece(i);
                }
